<?php
$id = intval($_GET['edit']);
$dir = WOLF_REFINESLIDE_FILES_DIR;
$path = WOLF_REFINESLIDE_FILES_URL;
$row = $wpdb->get_row("SELECT * FROM $wolf_refineslide_table WHERE id = $id");
$img = $row->img;
...
if( isset($_POST['updateimg']) ){
	...
	if( $_POST["x1"] != 'NaN' ){
		...
		$cropped = wolf_img_area_crop($dir.'/slides/'.$img, $dir.'/'.$img,$w,$h,$x1,$y1, $wolf_refineslide_width, $wolf_refineslide_height);

		$link = esc_url($_POST['link']);
		$capt_pos = esc_attr($_POST['caption_position']);


		$coordinates_array = array( $x1, $y1, $x2, $y2, $w, $h );
		$coordinates = serialize( $coordinates_array );
		$values = array('link' => $link, 'caption' => $_POST['caption'],  'caption_position' => $capt_pos, 'coordinates' => $coordinates, 'language_code' => ICL_LANGUAGE_CODE );
		$conditions = array( 'id' => $id);
		$values_types = array('%s');
		$conditions_types = array('%d');
		$wpdb->update($wolf_refineslide_table, $values, $conditions, $values_types, $conditions_types); 
...
<div class="wrap">
	...
	<form style="width:300px" action="<?php echo esc_url(admin_url('admin.php?page=wolf-refineslide-panel')); ?>&amp;edit=<?php echo $id; ?>" method="post">
		<?php wp_nonce_field('wolf_edit_refineslide_image', 'wolf_edit_refineslide_image_nonce'); ?>
		...
		<p>
			<label for="caption_position"><?php _e('Caption Position', 'wolf'); ?>:</label><br>
			<select name="caption_position">
				<option value="bottom-right" <?php echo ($row->caption_position == 'bottom-right') ? 'selected="selected"' : ''; ?>><?php _e('bottom right', 'wolf'); ?></option>
				<option value="bottom-left" <?php echo ($row->caption_position == 'bottom-left') ? 'selected="selected"' : ''; ?>><?php _e('bottom left', 'wolf'); ?></option>
				<option value="top-right" <?php echo ($row->caption_position == 'top-right') ? 'selected="selected"' : ''; ?>><?php _e('top right', 'wolf'); ?></option>
				<option value="top-left" <?php echo ($row->caption_position == 'top-left') ? 'selected="selected"' : ''; ?>><?php _e('top left', 'wolf'); ?></option>
			</select><br>
		</p>
	...
	<p><a href="<?php echo esc_url(admin_url('admin.php?page=wolf-refineslide-panel')); ?>"><?php _e('back to the main page', 'wolf'); ?></a></p>
</div>
<script type="text/javascript">

function preview(img, selection) { 
	var scaleX = <?php echo $wolf_refineslide_width;?> / selection.width; 
	var scaleY = <?php echo $wolf_refineslide_height;?> / selection.height; 
...
jQuery(window).load(function () { 
	...
	aspectRatio: '1:<?php echo $wolf_refineslide_height/$wolf_refineslide_width;?>', 
	x1: <?php echo $x1 ?>, 
	y1: <?php echo $y1 ?>,
	x2: <?php echo $x2 ?>, 
	y2: <?php echo $y2 ?>,
