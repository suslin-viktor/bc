...
if ( ! class_exists( 'Wolf_Video_Thumbnail' ) ) {
			...
			if ( ! function_exists( 'curl_init' ) ) {
				...
				if ( curl_error( $ch ) != null ) {
					$output = new WP_Error(
						...
						sprintf(
							__(
								'Error retrieving video information from the URL <a href="%1$s">%1$s</a>.<br />
								<a href="%2$s">Libcurl error</a> %3$s: <code>%4$s</code>. 
								If opening that URL in your web browser returns anything else than an error page, 
								the problem may be related to your web server and might be something your host administrator can solve.', 
								'wolf' 
							),
							$videoinfo_url,
		...
		public function render_metabox() {
			...
			if ( get_post_status() == 'publish' || get_post_status() == 'private' ) {
				...
				if ( isset( $video_thumbnail ) && $video_thumbnail != '' ) {
					...
					echo $this->get_text( 'not_published' );
		...
		public function video_thumbnail_script() {
			...
			function video_thumbnail_reset( id ) {
				...
				var removeFeaturedImg = '<p id="video-custom-remove" class="hide-if-no-js"><a href="#" id="remove-post-thumbnail" onclick="wolf_thumbnail_delete( ' + id + ' );return false;"><?php _e( 'Remove featured image', 'wolf' ); ?></a></p>';
					var resetText = '<?php echo $this->get_text( 'reset' ); ?>';
					var deleteText = '<?php echo $this->get_text( 'delete' ); ?>';			
			...
			function video_thumbnail_delete( id ) {
				...
				var message = '<?php echo $this->get_text( 'has_video' ); ?>';
				var generateText = '<?php echo $this->get_text( 'generate' ); ?>';
	...
	$wolf_portfolio_do_video_thumbnails = new Wolf_Video_Thumbnail;
